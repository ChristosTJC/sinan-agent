...
当前状态：Alpha。已完成 no-hardware smoke、dry-run 端到端验证、硬件黄金路径命令（`sinan golden-path`，默认只读 dry-run）和 593 个单元/集成测试，覆盖率达 64%。真实板卡 HIL 闭环验证待社区贡献或后续硬件环境补齐——欢迎插上板卡跑 `sinan run` 反馈结果。定位：**ready for hardware validation**。
